export const profileData = {
  name: "Jorge Lozano Fortes",
  role: "Full Stack Developer",
  profile: "Soy un desarrollador Full Stack altamente motivado y creativo con más de 10 años de experiencia en la creación de aplicaciones web y herramientas de software. Tengo experiencia en programación en lenguajes como PHP y JavaScript, y estoy familiarizado con frameworks como Laravel y Vue.js. También tengo habilidades en diseño de producto, UX/UI, y arquitectura de software. Me destaco por mi capacidad de trabajar en equipo, mi adaptabilidad y mi capacidad para resolver problemas de manera efectiva. Además, tengo experiencia en liderar y mentorizar a nuevos miembros del equipo.",
  experience: [
    {
      company: "Salutic",
      position: "Full Stack Developer",
      period: "Oct 2019 - Presente",
      description: "Desarrollo de aplicación web del sector salud utilizando PHP con el framework Laravel, MySQL con arquitectura hexagonal en entorno ágil. Revisión de código, buenas prácticas y aseguramiento de calidad mediante pruebas. Mentorización de nuevos empleados."
    },
    {
      company: "Nexius.es",
      position: "Co-fundador / Programador Front-end & Backend / Diseño de producto",
      period: "Abr 2012 - Oct 2019",
      description: "Co-fundador de empresa de desarrollo de software especializada en diseño y desarrollo de productos digitales innovadores. Encargado del diseño de producto, gestión del ciclo de vida completo del producto, y desarrollo Front-end UI/UX y Backend."
    },
    {
      company: "Avanade",
      position: "Business Analyst",
      period: "May 2015 - Abr 2017",
      description: "Desarrollo y mantenimiento de proyectos con Microsoft Dynamics AX. Mantenimiento de aplicaciones en VB6 y Oracle."
    },
    {
      company: "Colegio Oficial de Arquitectos de Málaga",
      position: "Desarrollador de Software y Soporte Técnico",
      period: "Oct 2006 - Mar 2014",
      description: "Soporte técnico a arquitectos para visado digital de proyectos. Desarrollo de aplicaciones y utilidades para el visado digital. Mantenimiento de la web oficial del colegio."
    },
    {
      company: "Ayuntamiento de Vélez-Málaga",
      position: "Juventud",
      period: "2003 - 2004",
      description: "Gestión de programas de juventud."
    }
  ],
  education: [
    {
      institution: "IES Juan de la Cierva",
      degree: "Técnico Superior en Administración de Sistemas Informáticos",
      period: "2001 - 2003"
    },
    {
      institution: "IES Juan de la Cierva",
      degree: "Técnico especialista en Informática de Gestión",
      period: "1997 - 2000"
    },
    {
      institution: "Curso Social Manager",
      degree: "Social Manager",
      period: "2013"
    }
  ],
  certifications: [
    "Curso de Configuración de Entorno de Desarrollo en Windows",
    "Curso de ECMAScript: Historia y Versiones de JavaScript",
    "Curso de NPM: Gestión de Paquetes y Dependencias en JavaScript",
    "Curso de React.js",
    "Curso de Gestión de Equipos Ágiles"
  ],
  techStack: [
    { name: "Vue.js", icon: "vue" },
    { name: "Laravel", icon: "laravel" },
    { name: "PHP", icon: "php" },
    { name: "JavaScript", icon: "javascript" },
    { name: "TypeScript", icon: "typescript" },
    { name: "Tailwind CSS", icon: "tailwind" },
    { name: "Vite", icon: "vite" },
    { name: "MySQL", icon: "mysql" },
    { name: "Git", icon: "git" },
    { name: "Figma", icon: "figma" }
  ],
  languages: [
    { name: "Español", level: "Nativo" },
    { name: "Inglés", level: "Professional Working" }
  ],
  contact: {
    email: "j-lozano@hotmail.es",
    phone: "666969888",
    location: "Málaga, Andalucía, España",
    linkedin: "https://www.linkedin.com/in/jlozanofortes",
    website: "https://jorgelozano.dev"
  }
};